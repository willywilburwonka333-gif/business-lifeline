...
export function BusinessLifeline() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/analyse", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ data, metrics: base.metrics }) });
      if (response.ok) { const result = await response.json() as { analysis?: AiAnalysis }; if (result.analysis) next = { ...base, aiAnalysis: result.analysis, aiStatus: "ready" }; }
    } catch { /* deterministic report remains available */ }
    setReport(next); localStorage.setItem(STORAGE_KEY, JSON.stringify({ data, report: next })); setView("report"); setAnalysing(false); window.scrollTo(0, 0);
  };
