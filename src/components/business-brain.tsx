...
export function BusinessBrain({ saved }: { saved: SavedReport }) {
  ...
  async function ask(event: FormEvent) {
    ...
    if (!aiConsent) {
      setError("Tick the AI consent box above before asking Business Brain.");
  ...
  return (
    <section className="business-brain" aria-labelledby="business-brain-title">
      ...
      <div className="brain-conversation" aria-live="polite">{conversation.length === 0 && <div className="brain-empty"><strong>No question asked yet.</strong><p>Start with one decision that is keeping you awake. Business Brain will separate what the figures support, what requires judgement and what should be verified professionally.</p></div>}{conversation.map((item, index) => <article className="brain-answer" key={`${item.question}-${index}`}><p className="brain-question"><span>You asked</span>{item.question}</p><div className="brain-main-answer"><span>{sourceLabel(item.source)}</span><p>{item.answer.answer}</p></div><div className="brain-answer-grid"><section><h4>Facts used</h4><ul>{item.answer.reasoningSummary.map((line) => <li key={line}>{line}</li>)}</ul></section><section><h4>Recommended next steps</h4><ol>{item.answer.nextSteps.map((line) => <li key={line}>{line}</li>)}</ol></section>{item.answer.watchOutFor.length > 0 && <section><h4>Risks and unknowns</h4><ul>{item.answer.watchOutFor.map((line) => <li key={line}>{line}</li>)}</ul></section>}</div><aside className={item.answer.professionalHelp.recommended ? "brain-escalation warning" : "brain-escalation"}><strong>{item.answer.professionalHelp.recommended ? "Professional help recommended" : "Professional judgement point"}</strong><p>{item.answer.professionalHelp.type}: {item.answer.professionalHelp.reason}</p></aside></article>)}</div>
